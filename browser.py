...
class MainWindow(QMainWindow):
    ...
    def __init__(self, options, parent = None):
        ...
        self.configuration = ConfigObj(self.options.config_file)
        if DEBUG:
    ...
    def build_ui(self, options, configuration):
        self.startUrl = options.url or configuration.get("start_url") or "about:blank"
        inactivity_timeout = options.timeout or int(configuration.get("timeout") or 0)
        self.icon_theme = options.icon_theme or configuration.get("icon_theme") or None
        ...
        if options.isFullscreen is True:
            self.showFullScreen()
        if options.noNav is not True:
            ...
            if configuration.get("bookmarks"):
                for bookmark in configuration.get("bookmarks").iteritems():
                    if DEBUG:
